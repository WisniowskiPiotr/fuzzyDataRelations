...
namespace fuzzyDataRelations
{
    ...
    public class FuzzyEngine
    {
        ...
        private double Parse(string text)
        {
            ...
            if (!text.StartsWith("("))
            {
                string[] MembershipName = text.Split();
                return this.linguisticVariableCollection.Find(MembershipName[0]).Fuzzify(MembershipName[2]);
            }
        ...
        private double Evaluate(string text)
        {
            ...
            for (int i = 0; i <= ((MembershipName.Length / 2) + 1); i = i + 2)
            {
                double tokenValue = Convert.ToDouble(MembershipName[i]);
                ...
                if ((i + 1) < MembershipName.Length)
                    connective = MembershipName[i + 1];
            }
        ...
        public double Defuzzify()
        {
            ...
            foreach (FuzzyRule fuzzyRule in this.fuzzyRuleCollection)
            {
                ...
                string[] MembershipName = fuzzyRule.Text.Split();
                MembershipFunction membershipFunction = this.GetConsequent().MembershipFunctionCollection.Find(MembershipName[MembershipName.Length - 1]);
